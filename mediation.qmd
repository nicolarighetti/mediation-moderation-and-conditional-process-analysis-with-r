...
Based on statistical research, the recommendation is, in general, to avoid the normal theory test for indirect effect(s) approach, and to interpret the confidence intervals. This approach would be better in several empirical circumstances, and more powerful in detecting statistical significance.
...
Resampling with replacement is a resampling method that create new samples by choosing, in a random way, the cases that compose an initial sample. The particularity is that *the* *same case can be chosen more than once*:
...
Read the following paper and replicate the study 1 analysis using the provided data. Write a short report with diagrams, tables, and the interpretation of results.
